...
- Compare against strong baselines, not a strawman. A raw transcript is the weakest competitor;
 ...
 full cost counted.
- Do not design the benchmark so ASTP wins. Include small, clean, and easy contexts where ASTP
...
- ASTP-Bench harness: ten hand-authored scenarios, episode logging, real tokenizers, a
...
1. Summary and RAG baselines. Add a `summary` condition (an LLM condenses Agent A's context;
 ...
 fetched by vector retrieval). This is the credibility unlock, it tests ASTP against the
...
3. Failure taxonomy. Categorize every failure (information omitted, hallucinated, wrong/stale
 decision retained, artifact unavailable, retrieval failure, ASTP extraction/linking failure,
...
1. Automatic extraction. Build the extractor that turns a real repository or agent transcript
 into an ASTP graph. This is the central gate: it unlocks external benchmarks and real agent
 capture, and its recall is the system's ceiling. It should have its own fidelity benchmark.
2. External OSS benchmark adapters. Adapt existing suites so ASTP is measured on independent
...
1. Multi-agent / swarm benchmarks. Software-engineering, research, data-analysis, long-horizon
 ...
 pass). Vary swarm size (2 to 20 agents) and measure whether ASTP's value grows with it.
2. Context-size stress. Test up to 128K-256K accumulated context; the hypothesis is that
 transcript and summary approaches degrade while ASTP stays comparatively stable.
...
5. Cross-model interoperability at scale. Same-model, same-family, and cross-family handoffs
 (open ↔ proprietary) to test whether ASTP is a genuine interoperability layer.
...
1. Task success vs. context size, for transcript / summary / RAG / ASTP.
...
The hypothesis they test: ASTP stays comparatively stable as context grows larger and noisier,
...
exact ASTP configuration, so others can reproduce and extend the results.
...
would show ASTP's advantage growing as context becomes larger, noisier, and more distributed.
It is an open question, not a settled result.

In one line: build ASTP, benchmark realistic agent workflows against strong baselines, measure
