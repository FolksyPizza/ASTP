...
A plain-language guide to what ASTP is and how it works, for readers who are new to the ideas.
...
## The problem ASTP is trying to solve
...
ASTP is a third answer: **keep the complete state, but only send what's relevant now, and let the
receiver fetch the original details on demand.**

## The core idea in one picture

```
Agent A's work ──▶ ASTP builds a structured "map" of the state ──▶ send only the relevant part
 │
 └──▶ the full detail stays stored, fetchable on request
```

ASTP's motto: **preserve everything, propagate selectively, retrieve precisely.**

## What "structured state" means: nodes and edges

ASTP stores state as a **graph**. A graph is just **things (nodes)** connected by **relationships
(edges)**. If you've seen a diagram of boxes joined by arrows, that's a graph.

- **Nodes** are the important things. ASTP has four kinds:
...
- **Edges** are the relationships, and they're *labeled*. A few labels ASTP uses:
...
ASTP uses the same idea with its `calls` and `depends_on` edges. When the task is "change
...
ASTP never edits state in place. Instead it keeps an **append-only event log**, a list of small
...
## How ASTP decides what to send: the selector

When it's time to hand off, ASTP runs the **selector**. Today the Core selector is a simple,
...
If the receiver actually needs the full source, it asks, it emits `RETRIEVE <id>`, and ASTP
...
ASTP is a research prototype. It's evaluated by having a second agent actually do the task from
each kind of handoff (transcript, summary, ASTP) and checking two things: **did the task
...
[EXPERIMENTS.md](EXPERIMENTS.md), including a case where ASTP *fails*, which is kept on purpose.
